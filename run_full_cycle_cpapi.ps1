...
#   0b) [ОПЦІЯ] Скидання broker_log.json (явна команда)
...
#   5a) Синхронізація preview -> orders
#   5b) Резолвінг conid для cleanup символів
...
#   -ResetBrokerLog : явно скидає broker_log.json перед кроком 4
# ============================================================
param(
    [switch]$ResetBrokerLog = $false
)
...
foreach ($cfg in @("optimal", "aggressive")) {
    ...
    $toDelete = @(
        "orders.csv", "target_book.csv", "fills.csv",
    ...
    Write-Host "  [$cfg] clean OK (portfolio_state.json + broker_log.json preserved)" -ForegroundColor Green
}

# ============================================================
# КРОК 0b — [ОПЦІЯ] Скидання broker_log.json
# Виконується тільки якщо передано прапор -ResetBrokerLog.
# Без цього прапора broker_log зберігається між запусками —
# idempotency активна, повторний запуск не відправить ті
# самі ордери вдруге.
# ============================================================
Write-Host ""
if ($ResetBrokerLog.IsPresent) {
    Write-Host "=== STEP 0b: RESET BROKER LOG (explicit) ===" -ForegroundColor Yellow
    foreach ($cfg in @("optimal", "aggressive")) {
        $logPath = "$ROOT\artifacts\execution_loop\$cfg\broker_log.json"
        if (Test-Path $logPath) {
            Remove-Item $logPath -Force
            Write-Host "  [$cfg] broker_log.json deleted" -ForegroundColor Yellow
        } else {
            Write-Host "  [$cfg] broker_log.json not found -- skip" -ForegroundColor DarkGray
        }
    }
    Write-Host "  broker_log reset complete" -ForegroundColor Yellow
} else {
    Write-Host "=== STEP 0b: BROKER LOG preserved (idempotency active) ===" -ForegroundColor DarkGray
    Write-Host "  Use -ResetBrokerLog flag to explicitly reset broker_log.json" -ForegroundColor DarkGray
}
...
# ============================================================
# КРОК 2 — Execution loop (fractional mode)
# ============================================================
Write-Host ""
Write-Host "=== STEP 2: EXECUTION LOOP (fractional mode) ===" -ForegroundColor Cyan
...
$env:ALLOW_FRACTIONAL_SHARES                  = "1"
$env:FRACTIONAL_MODE                          = "fractional"
...
# ============================================================
# КРОК 3 — CPAPI handoff (live BBO + conid_cache)
# ============================================================
Write-Host ""
...
# RESET_BROKER_LOG=0 : idempotency активна за замовчуванням.
# Для скидання запускайте з -ResetBrokerLog.
# ============================================================
Write-Host ""
...
# ============================================================
# КРОК 5a — Синхронізація preview -> orders
# ============================================================
Write-Host ""
...
if ($ResetBrokerLog.IsPresent) {
    Write-Host "  [NOTE] broker_log.json was reset this run (-ResetBrokerLog)" -ForegroundColor Yellow
}
