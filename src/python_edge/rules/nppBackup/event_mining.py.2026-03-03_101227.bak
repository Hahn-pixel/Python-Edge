...
def _quantile_thresholds(s: pd.Series) -> Dict[str, float]:
    ...
    if len(s2) < 50:
        q20 = float(s2.quantile(0.2)) if len(s2) else 0.0
        q50 = float(s2.quantile(0.5)) if len(s2) else 0.0
        q80 = float(s2.quantile(0.8)) if len(s2) else 0.0
        return {"q20": q20, "q50": q50, "q80": q80}
    return {"q20": float(s2.quantile(0.2)), "q50": float(s2.quantile(0.5)), "q80": float(s2.quantile(0.8))}
...
def _canonical_signature(direction: str, conds: List[Tuple[str, str, str]]) -> str:
    """
    Cross-fold comparable signature. Uses quantile TAG (q20/q50/q80) not numeric threshold.
    meta_key is like "atr_pct__q50" => tag="q50".
    """
    parts = []
    for feat, op, meta_key in conds:
        # extract tag after last "__"
...
def _permutation_sanity(df_train: pd.DataFrame, rules: List[Rule], cfg: EventMiningConfig) -> Dict[str, float]:
    ...
    return {
        ...
        "perm_topk_mean": float(np.mean(arr)) if arr.size else float("nan"),
        "perm_topk_p95": float(np.quantile(arr, 0.95)) if arr.size else float("nan"),
    }


def mine_event_rules(df_train: pd.DataFrame, feature_cols: List[str], cfg: EventMiningConfig) -> Tuple[List[Rule], Dict[str, RuleStats], Dict[str, float]]:
    ...
    for k in range(cfg.max_rules_try):
        ...
        rid = f"E{direction[0].upper()}{k:05d}"
        candidates.append(_make_rule(rng, rid, direction, feats, thrs))

    # dedup by CANONICAL signature
