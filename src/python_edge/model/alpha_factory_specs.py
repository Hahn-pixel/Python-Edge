...
# ---------------------------------------------------------------------------
# Base signal registry
# ---------------------------------------------------------------------------
# New in Stage 4:
#   close_location  — where close lands within the day's high-low range
#                     = (close - low) / (high - low)
#                     captures intraday price pressure direction
#   gap_abs         — absolute gap magnitude (open/prev_close - 1) clipped
#                     captures gap size regardless of direction
#   turnover_z      — dollar volume z-score (20d rolling)
#                     distinct from rel_volume: captures dollar-weighted activity
...
BASE_SIGNAL_REGISTRY: tuple[BaseSignalSpec, ...] = (
    ...
    # --- Stage 4 additions ---
    BaseSignalSpec("close_location",   "close_location",  "close_location"),
    BaseSignalSpec("gap_abs",          "gap_abs",         "gap_abs"),
    BaseSignalSpec("turnover_z",       "turnover_z",      "turnover_z"),
)
...
# ---------------------------------------------------------------------------
# New in Stage 4:
#   ivol_ratio  — short-term (5d) vs long-term (20d) vol ratio
#                 > 1.0 = vol expanding, < 1.0 = vol compressing
#                 useful as vol-regime gate for ivol_20d and vol_compression
#   mom_sign    — sign of 20d momentum (+1 / -1)
#                 trend direction gate; conditions signals on trend alignment
...
MODULATOR_REGISTRY: tuple[ModulatorSpec, ...] = (
    ...
    # --- Stage 4 additions ---
    ModulatorSpec("ivol_ratio","ivol_ratio",     "volatility"),
    ModulatorSpec("mom_sign",  "mom_sign",       "momentum"),
)
...
# Wave 1-3 seeds
# ---------------------------------------------------------------------------
# Stage 4: extend modulators for new base signals + add new signals to seeds
...
WAVE1_REGISTRY: tuple[FamilySeed, ...] = (
    ...
    FamilySeed("ivol_20d",          "ivol_20d",          ("liq", "volshock", "ivol_ratio")),
    FamilySeed("vol_compression",   "vol_compression",   ("liq", "range_comp", "ivol_ratio")),
    FamilySeed("intraday_rs",       "intraday_rs",       ("breadth", "rvol")),
    FamilySeed("intraday_pressure", "intraday_pressure", ("liq", "breadth")),
    # --- Stage 4 new families ---
    FamilySeed("close_location",    "close_location",    ("breadth", "rvol", "volshock")),
    FamilySeed("gap_abs",           "gap_abs",           ("rvol", "volshock", "breadth")),
    FamilySeed("turnover_z",        "turnover_z",        ("breadth", "mom_sign", "liq")),
)
...
# ---------------------------------------------------------------------------
# Survivor expansion waves (unchanged Wave 4-7)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Survivor priority — reordered based on Stage 3 diagnostics
# ---------------------------------------------------------------------------
# Previous order: rev1, pressure, gap, mom3, momentum_20d, intraday_rs, ...
# New order: families with highest mean_ic from console222/223 diagnostics:
#   vol_compression (0.0098), overnight_drift_20d (0.0098), ivol_20d (0.0053),
#   mom3 (0.0047), gap (0.0033), intraday_pressure (0.0024), pressure (0.0026)
#   Stage 4 new families appended at end (no historical IC yet)
...
SURVIVOR_DEFAULT_PRIORITY: tuple[str, ...] = (
    ...
    "rev1",
    "str_3d",
    # Stage 4 new — no IC history yet, placed at end
    "close_location",
    "gap_abs",
    "turnover_z",
)
...
# ---------------------------------------------------------------------------
# Recipe builders (unchanged logic)
# ---------------------------------------------------------------------------
        ...
        for modulator in seed.modulators:
            for regime in WAVE1_REGIMES:
                ...
                _append_unique(
                    out, seen,
                    AlphaRecipe(
                        name=name, left=seed.base_signal, modulator=modulator,
                        transform="raw", regime=regime, interaction="regime",
                        lag=0, family=seed.family, wave="wave1",
                        parents=(seed.family,), source="registry:wave1",
                    ),
                )
    return tuple(out)
            ...
            for transform_seed in WAVE2_REGISTRY:
                for regime in transform_seed.regimes:
                    ...
                    _append_unique(
                        ...
                        AlphaRecipe(
                            name=name, left=seed.base_signal, modulator=modulator,
                            transform=transform_seed.transform, regime=regime,
                            interaction=transform_seed.interaction, lag=0,
                            family=seed.family, wave="wave2",
                            parents=(seed.family,), source="registry:wave2",
                        ),
                    )
    return tuple(out)
            ...
            for temporal_seed in WAVE3_REGISTRY:
                for regime in temporal_seed.regimes:
                    ...
                    _append_unique(
                        ...
                        AlphaRecipe(
                            name=name, left=seed.base_signal, modulator=modulator,
                            transform=temporal_seed.transform, regime=regime,
                            interaction=temporal_seed.interaction, lag=temporal_seed.lag,
                            family=seed.family, wave="wave3",
                            parents=(seed.family,), source="registry:wave3",
                        ),
                    )
    return tuple(out)
