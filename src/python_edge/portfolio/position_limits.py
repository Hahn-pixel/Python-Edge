...
def cap_single_name_weight(
    ...
    if side_col not in out.columns:
        raise RuntimeError(f"cap_single_name_weight: missing side_col={side_col}")
    s = pd.to_numeric(out[side_col], errors="coerce").fillna(0.0)
    out[side_col] = s.clip(lower=-cap_abs_weight, upper=cap_abs_weight)
    return out
...
def normalize_gross_exposure(
    ...
    for dt, idx in out.groupby("date").groups.items():
        gross = float(pd.to_numeric(out.loc[idx, side_col], errors="coerce").abs().sum())
        if gross <= 0.0:
            continue
        out.loc[idx, "weight"] = pd.to_numeric(out.loc[idx, side_col], errors="coerce").fillna(0.0) * (gross_target / gross)
    return out