...
from python_edge.broker.ibkr_adapter_support import (
    apply_working_order_policy,
    classify_outcome,
    clip_limit_from_202,
    collect_app_errors,
    find_error_code,
...
def prepare_orders(config_name: str, df: pd.DataFrame, symbol_map: Dict[str, str]) -> List[PreparedOrder]:
    ...
    for _, row in df.iterrows():
        ...
        idempotency_key = make_idempotency_key(config_name, row_dict, broker_symbol)
        prepared.append(
            PreparedOrder(
                config=config_name,
...
def retry_buy_sell_bps(attempt_idx: int) -> Tuple[float, float]:
    ...
    buy_bps = min(float(IB_LMT_PRICE_BUY_BPS), float(IB_RETRY_202_BUY_BPS_STEPS[buy_idx]))
    sell_bps = min(float(IB_LMT_PRICE_SELL_BPS), float(IB_RETRY_202_SELL_BPS_STEPS[sell_idx]))
    return buy_bps, sell_bps


def first_fractional_issue(prepared: PreparedOrder, broker_errors: List[BrokerErrorInfo]) -> OrderIssue | None:
    if not is_fractional_qty(prepared.qty):
        return None
    code_set = set(IB_FRACTIONAL_REJECT_CODES)
...
def first_generic_issue(broker_errors: List[BrokerErrorInfo]) -> OrderIssue | None:
...
def entry_from_ib(
    ...
    rounded_limit_price: float | None,
    retry_count: int,
    retry_mode: str,
    outcome_override: str,
    contract_meta_debug: Dict[str, Any],
) -> dict:
...
def run_one_config(app: IBKRApp, paths: ConfigPaths, symbol_map: Dict[str, str], req_id_seed: int) -> int:
    ...
    outcome_counters: Dict[str, int] = {"filled_now": 0, "partial": 0, "working": 0, "working_carry": 0, "cancelled_after_ttl": 0, "failed": 0, "unknown": 0}
    ...
    for prepared0 in prepared_orders:
        ...
        try:
            prepared, contract, req_cursor, contract_meta_debug = resolve_contract_metadata(
                app=app,
                prepared=prepared0,
                req_id_seed=req_cursor,
                timeout_sec=IB_TIMEOUT_SEC,
                retries=IB_CONTRACT_DETAILS_RETRIES,
                retry_sleep_sec=IB_CONTRACT_DETAILS_RETRY_SLEEP_SEC,
                allow_primary_exchange_fallback=IB_ALLOW_PRIMARY_EXCHANGE_FALLBACK,
                allow_submit_without_contract_details=IB_ALLOW_SUBMIT_WITHOUT_CONTRACT_DETAILS,
                build_contract=build_contract,
                normalize_min_tick=normalize_min_tick,
                min_abs=IB_LMT_PRICE_MIN_ABS,
                to_float=to_float,
            )
            ...
            for attempt_idx in range(max_attempts):
                ...
                order, raw_limit_price, rounded_limit_price = build_order(prepared, buy_bps, sell_bps)
                ...
                if IB_POLL_AFTER_SUBMIT:
                    ib_entry = poll_order_status(
                        app=app,
                        ib_order_id=ib_order_id,
                        initial_entry=ib_entry,
                        attempts=IB_POLL_ATTEMPTS,
                        sleep_sec=IB_POLL_SLEEP_SEC,
                        label="after_submit",
                        refresh_open_orders=refresh_open_orders,
                        print_mode=IB_POLL_VERBOSE,
                        print_every=IB_POLL_PRINT_EVERY,
                        to_float=to_float,
                    )

                broker_errors = collect_app_errors(app, error_cursor_before_submit, [int(ib_order_id), int(req_cursor)], utc_now_iso)
                ...
                if issue is None or status_lower in ACCEPTED_STATUSES:
                    if IB_FINAL_STATUS_POLL_ENABLED:
                        ib_entry = poll_order_status(
                            ...
                            ib_order_id=ib_order_id,
                            initial_entry=ib_entry,
                            attempts=IB_FINAL_STATUS_POLL_ATTEMPTS,
                            sleep_sec=IB_FINAL_STATUS_POLL_SLEEP_SEC,
                            label="final_status",
                            refresh_open_orders=refresh_open_orders,
                            print_mode=IB_POLL_VERBOSE,
                            print_every=IB_POLL_PRINT_EVERY,
                    ...
                    ib_entry, outcome_override, _ = apply_working_order_policy(
                        app=app,
                        ib_order_id=ib_order_id,
                        current_entry=ib_entry,
                        working_policy=normalize_working_policy(),
                        ttl_sec=IB_WORKING_ORDER_TTL_SEC,
                        cancel_attempts=IB_WORKING_CANCEL_POLL_ATTEMPTS,
                        cancel_sleep_sec=IB_WORKING_CANCEL_POLL_SLEEP_SEC,
                        refresh_open_orders=refresh_open_orders,
                        print_mode=IB_POLL_VERBOSE,
                        print_every=IB_POLL_PRINT_EVERY,
                        to_float=to_float,
                    )
                    final_entry = entry_from_ib(prepared, ib_entry, paths.orders_csv, buy_bps, sell_bps, raw_limit_price, rounded_limit_price, attempt_idx, retry_mode, outcome_override, contract_meta_debug)
                ...
                if retryable_202 is not None and attempt_idx + 1 < max_attempts:
                    retry_202_count += 1
                    clip_raw, clip_rounded = clip_limit_from_202(
                        prepared=prepared,
                        current_raw=raw_limit_price,
                        current_rounded=rounded_limit_price,
                        err202=retryable_202,
                        order_type=effective_order_type(),
                        min_abs=IB_LMT_PRICE_MIN_ABS,
                        clip_ticks=IB_RETRY_202_CLIP_TICKS,
                        compute_limit_price=compute_limit_price,
                    )
                    if clip_raw is not None and clip_rounded is not None:
                        cutoff, market_price = parse_202_limits(retryable_202.error_string)
                        print(
                            f"[BROKER][{paths.name}][RETRY_202_CLIP] symbol={prepared.symbol} side={prepared.order_side} qty={prepared.qty:.8f} "
                            f"attempt={attempt_idx + 1}/{max_attempts - 1} code=202 cutoff={(cutoff or 0.0):.4f} market_price={(market_price or 0.0):.4f} "
                            f"raw_limit_price={(raw_limit_price or 0.0):.4f} rounded_limit_price={(rounded_limit_price or 0.0):.4f} next_raw_limit_price={clip_raw:.4f} next_rounded_limit_price={clip_rounded:.4f} min_tick={prepared.min_tick:.6f}"
                        )
            ...
            if final_entry is not None:
                ...
                print(
                    ...
                    f"status={final_entry['status']} outcome={outcome} ib_order_id={final_entry['broker_order_id']} order_type={effective_order_type()} outside_rth={int(IB_OUTSIDE_RTH)} "
                    f"price_hint={prepared.price:.4f} raw_limit_price={(final_raw_limit_price or 0.0):.4f} rounded_limit_price={(final_rounded_limit_price or 0.0):.4f} min_tick={prepared.min_tick:.6f} retry_count_202={final_retry_count} retry_mode_202={final_retry_mode} contract_details_mode={contract_meta_debug.get('contract_details_mode', '')}"
                )
                continue
            ...
            error_entry = build_error_entry(prepared, paths, final_issue, final_buy_bps, final_sell_bps, final_raw_limit_price, final_rounded_limit_price, final_retry_count, final_retry_mode, contract_meta_debug)
...
def main() -> int:
    ...
    print(
        f"[CFG] contract_details_retries={IB_CONTRACT_DETAILS_RETRIES} contract_details_retry_sleep_sec={IB_CONTRACT_DETAILS_RETRY_SLEEP_SEC} "
        f"allow_primary_exchange_fallback={int(IB_ALLOW_PRIMARY_EXCHANGE_FALLBACK)} allow_submit_without_contract_details={int(IB_ALLOW_SUBMIT_WITHOUT_CONTRACT_DETAILS)}"
    )
