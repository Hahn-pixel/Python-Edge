...
def _refresh_live_alpha_snapshot_once() -> None:
    ...
    # Передаємо os.environ явно щоб дочірній процес бачив UNIVERSE_SNAPSHOT_FILE
    # та всі інші env змінні батьківського процесу.
    # Без env= subprocess отримує чистий системний env де може бути
    # застарілий UNIVERSE_SNAPSHOT_FILE з попередніх запусків.
    refresh_env = os.environ.copy()
    print(f"[REFRESH] live alpha snapshot via: {' '.join(cmd)}")
    print(f"[REFRESH] UNIVERSE_SNAPSHOT_FILE={refresh_env.get('UNIVERSE_SNAPSHOT_FILE', '(not set — will use script default)')}")
    completed = subprocess.run(cmd, cwd=str(ROOT), env=refresh_env)
